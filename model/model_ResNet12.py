...
class EmbeddingNet(nn.Module):
    def __init__(self, dim, n_class_train):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=None, out_channels=64, kernel_size=3, padding=1, bias=False),
                                   nn.BatchNorm2d(num_features=64),
                                   nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1, bias=False),
                                   nn.BatchNorm2d(num_features=64),
                                   nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, padding=1, bias=False),
                                   nn.BatchNorm2d(num_features=64))
        self.conv1_r = nn.Sequential(nn.Conv2d(in_channels=None, out_channels=64, kernel_size=3, padding=1, bias=False),
                                     nn.BatchNorm2d(num_features=64))
    ...
    def forward(self, x, batch_size):
        x = x.view(batch_size, 84, 84, 3)
        x = x.permute(0, 3, 1, 2).contiguous()  # shape = (batch size, 3, 84, 84)
